...
def Get_Path_installations_states_rundate():
    logging.info("run FtpScript2.py in order to get path from FTP")
    path,df_installations,df_states,rundate = FtpScript2.main()
...
def StoreDF():
    try:
        ...
        logging.info("working on setid : 02")
        lab_config = Get_Configuration()
        # Create a folder for states if it doesn't exist
        States_folder = os.path.join(path, "States_Set02")
        os.makedirs(States_folder)
        logging.info("make folder for States_Set01 in path : " + path)
        df_states.to_csv(os.path.join(States_folder, f'States_Set02.csv'), index=False)
        logging.info("save df_states as CSV for States_Set02 in path : " + States_folder)
        ...
        for lab_name in lab_config:
            ...
            # Save summary_df and unit_df as CSV files in the lab folder
        ...
        file_path_date = os.path.join(path, 'dateUpdate02.txt')
        with open(file_path_date, "w") as f:
            f.write(str(date))
        file_path_rundate = os.path.join(path, 'rundate02.txt')
...
def Get_DLL(lab):
    labs_dll = {
        'LAT_I_09': ["Payment"],
        'LAT_G_07': ["fuelcashieraccountability","fuelmtx","fuelcouponue","fuelepsilon1"],
        'LAT_P_16': ["cashoffice","endofday"],
        'LAT_B_02': ["fuel2","fuelpromotions"],
        'LAT_W_23': ["draweraccountability","twostores"],
        'LAT_O_15': ["receipt","datapattern","return","patronid"],
        'LAT_Q_17': ["cashieraccountability","toggletransaction"],
        'LAT_H_08': ["Tax"],
        'LAT_K_11': ["EPS","Organization","CustomerScale","AgeRestriction","PromotionsUE","FiPaymentUE"],
        'LAT_A_01': ["Office1"],
        'LAT_F_06': ["StoredValue","BRM"],
        'LAT_C_03': ["fuel1"],
        'LAT_D_04': ["fueldraweraccountability","manualfuel","fuel3"],
        'LAT_V_22': ["return1"],
        'LAT_S_19': ["Selling2","CatalogPricesPE"],
        'LAT_N_14': ["office","datetimemanipulator","individualwrapper"],
        'LAT_L_12': ["FiPayment","TenderItemRestriction","TenderExchange","FLSCO"],
        'LAT_M_13': ["selling"],
        'LAT_R_18': ["fuelepsilon1","fuelepsilon","fuel"],
        'LAT_U_21': ["selling1"],
        'LAT_T_20': ["R1","GovernmentProgram"],
        'LAT_J_10': ["product","epsilon","pharmacy","idm","coindispenser"],
    }
    ...
    for df in df_summary.values() :
        ...
        for row,name in df.iterrows():
            ...
            if df.at[row,'Type'] == 'Install_Test':
                ...
                duration += timedelta(hours=int(duration_parts[0]), minutes=int(duration_parts[1]),
                                     seconds=int(duration_parts[2]))
                #tests_dll += (df.at[row,'Test_Name']).split('.')[2]+", "
        data_summary.append({
            'Lab_Name' : LabName,
...
def main():
    currentpath = os.getcwd() + "\AnalyzeData2.log"
    if os.path.exists(currentpath):
        os.remove(currentpath)
    logging.basicConfig(level=logging.ERROR,
                        filename='AnalyzeData2.log',
                        filemode='w',
                        format=f'[%(levelname)s] %(asctime)s | : Line No. : %(lineno)d - %(message)s',
                        datefmt='%Y-%m-%d %H:%M:%S',
                        )
    global script_filename
    script_filename = 'AnalyzeData2.py'
    logging.info(f"------------------- {script_filename} --------------------")
    StoreDF()
    return "Done AnalyzeData 2 successfully"
#TODO:
# analyze the data
